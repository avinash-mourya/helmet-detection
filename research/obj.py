...
import cv2 as cv
from encode_decode.img_enc_dec import encodeImageIntoBase64
...
class Detector:
    ...
    def draw_box(self, img, classId, score, bbox, rows, cols):
        if score > 0.3:
            ...
            cv.putText(img, name, (int(x) - 2, int(y) - 5), cv.FONT_HERSHEY_COMPLEX, 0.7, color, 1)
            cv.putText(img, f"confidence:{str(confidence)}", (int(x) - 2, int(y) - 20), cv.FONT_HERSHEY_COMPLEX, 0.7,
                       color, 1)
            # cv.putText(img, f"distance from camera:{str(dist)}", (390, 440), cv.FONT_HERSHEY_COMPLEX, 0.4, (255, 0, 0),
            #            1)
    ...
    def detect(self,img):
        ...
        with tf.Session(graph=self.detection_graph) as sess:
            ...
            # Run the model
            (num_detections, scores, bboxes, pred_class) = sess.run(
            ...
            encodeImg = encodeImageIntoBase64("output.jpg")
            return encodeImg.decode("utf-8")
